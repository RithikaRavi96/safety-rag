...
# 🔎 Show and normalize headers to avoid hidden spaces/tabs
...
for i, row in df.iterrows():
    ...
    # Skip empty rows (must have at least one meaningful field)
    if not any([function_txt, hazard_txt, cause_txt, msg_txt]):
        continue
    kept_rows += 1

    # Build page_content (labels help semantic retrieval)
    parts = []
    ...
    # Codes & explicit key (retrieve now, generate later)
    ...
    # Store everything the chatbot needs
    metadata = {
        "row": int(i),
